...
export const useAuthStore = () => {
    ...
    const startCreating = async({name, email, password, photoURL = defaultPhoto}) => {
        dispatch(onChecking());
        try {
            const {data} = await cloudateAPI.post('/auth/new', {name, email, password, photoURL});
    ...
    const registerWithGoogle = async() => {
        ...
        try {
            ...
            const {data} = await cloudateAPI.post('/auth/new-google', {name: displayName, email, photoURL});
            localStorage.setItem('token', data.token);
            localStorage.setItem('token-init-date', new Date().getTime());
            dispatch(onLogin({name: data.name, uid: data.uid, photoURL: data.photoURL}));
        } catch (error) {
    ...
    const startLogin = async({email, password}) => {
        dispatch(onChecking());
        try {
            const {data} = await cloudateAPI.post('/auth', {email, password});
            localStorage.setItem('token', data.token);
            localStorage.setItem('token-init-date', new Date().getTime());
            dispatch(onLogin({name: data.name, uid: data.uid, photoURL: data.photoURL}));
        } catch (error) {
    ...
    const loginWithGoogle = async() => {
        ...
        try {
            ...
            const {data} = await cloudateAPI.post('/auth/google', {email});
            localStorage.setItem('token', data.token);
            localStorage.setItem('token-init-date', new Date().getTime());
            dispatch(onLogin({name: data.name, uid: data.uid, photoURL: data.photoURL}));
        } catch (error) {
    ...
    const checkAuthToken = async() => {
        ...
        try {
            const {data} = await cloudateAPI.get('/auth/renew');
            localStorage.setItem('token', data.token);
            localStorage.setItem('token-init-date', new Date().getTime());
            dispatch(onLogin({name: data.name, uid: data.uid, photoURL: data.photoURL}));
        } catch (error) {
